...
class SupplierController {
    ...
    public searchs = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const result = await this.service.searchs(key, supplier_id, branch_id, pageInt, limitInt, seller_id, user_id);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
    ...
    public updateProfile = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const model = customerDto as any;
            model.created_id = req.id;
            const result = await this.service.updateProfile(model);
            if (result instanceof Error && result.field)
                return sendResponse(res, result.status, result.message, null, result.field);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
    ...
    public checkQuantity = async (req: Request, res: Response, next: NextFunction) => {
        const {product_id, quantity, branch_id} = req.body as any;
        try {
            const result = await this.service.checkQuantity(quantity, product_id, branch_id);
            if (result instanceof Error && result.field)
                return sendResponse(res, result.status, result.message, null, result.field);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
    ...
    public exportWarehouse = async (req: Request, res: Response, next: NextFunction) => {
        const {product_id, quantity, branch_id} = req.body as any;
        try {
            const result = await this.service.exportWarehouse(product_id, branch_id, quantity);
            if (result instanceof Error && result.field)
                return sendResponse(res, result.status, result.message, null, result.field);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
    ...
    public warehousereport = async (req: Request, res: Response, next: NextFunction) => {
        const {branch_id, page, limit, key} = req.query as any;
        const seller_id = req.seller_id as number
        try {
            const result = await this.service.getInventorySummary(key, branch_id, seller_id, limit, page);
            return sendResponse(res, 200, message.CHECK_SUCCESS, result);
        } catch (error) {
            next(error);
        }
    ...
    public warehouseReportDetail = async (req: Request, res: Response, next: NextFunction) => {
        const {branch_id, page, limit, id, fromDate, toDate} = req.query as any;
        const seller_id = req.seller_id as number
       //console.log(branch_id, page, limit, id)
        try {
            const result = await this.service.getMaterialDetails(branch_id, id, seller_id, page, limit, fromDate, toDate);
            if (result instanceof Error && result.field)
                return sendResponse(res, result.status, result.message, null, result.field);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
