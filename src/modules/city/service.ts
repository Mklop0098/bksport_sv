...
class CityService {
    ...
    public create = async (model: CreateDto) => {
        ...
        const result = await database.executeQuery(`insert into ${this.tableName} (name, publish, created_at, updated_at) values (?, ?, ?, ?)`, [model.name, publish, created_at, updated_at]);
    ...
    public searchs = async (key: string, name: string, publish: boolean, page: number, limit: number) => {
        let query = `select city.*, (select count(district.id) from district where district.city_id = city.id) as totalDistrict from ${this.tableName} city where 1=1 `;
    ...
    public importDataJson = async () => {
        const projectRoot = process.cwd()
        const jsonPath = path.join(projectRoot, process.env.VIETNAM_PROVINCE_PATH as string);
        try {
            ...
            for (const item of data) {
                const created_at = new Date()
                const updated_at = new Date()
                const city = await database.executeQuery(`insert into ${this.tableName} (name, publish, created_at, updated_at) values (?, ?, ?, ?)`, [item.name, 1, created_at, updated_at]);
                for (const district of item.district) {
                    const districtCreated_at = new Date()
                    const districtUpdated_at = new Date()
                    const districtId = await database.executeQuery(`insert into district (name, publish, created_at, updated_at, city_id) values (?, ?, ?, ?, ?)`, [district.pre + ' ' +district.name, 1, districtCreated_at, districtUpdated_at, (city as any).insertId]);
                    for (const ward of district.ward) {
                        const wardCreated_at = new Date()
                        const wardUpdated_at = new Date()
                        result = await database.executeQuery(`insert into ward (name, publish, created_at, updated_at, district_id, city_id) values (?, ?, ?, ?, ?, ?)`, [ward.pre + ' ' + ward.name, 1, wardCreated_at, wardUpdated_at, (districtId as any).insertId, (city as any).insertId]);
                    }
                }
            };
            return true
        } catch (error) {
            return new HttpException(500, errorMessages.IMPORT_FAILED);
        }
    }
