...
class RoleService {
    ...
    public create = async (model: CreateDto) => {
        const existQuery = `SELECT id FROM ${this.tableName} WHERE name = ? and seller_id = ?`
        const exist = await database.executeQuery(existQuery, [model.name, model.seller_id]) as RowDataPacket
        if (exist.length > 0) {
        ...
        if (code && code.length > 0) {
            ...
            code = await generateCodePrefixChar(this.tableName, 'RN', 8)
        }
        const created_at = new Date()
        const updated_at = new Date()
        const query = `insert into ${this.tableName} (name, publish, created_id, created_at, updated_at, code, seller_id) values (?, ?, ?, ?, ?, ?, ?)`
        const result = await database.executeQuery(query, [model.name, model.publish ?? 1, model.created_id, created_at, updated_at, code, model.seller_id]);
