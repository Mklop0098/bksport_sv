...
import DeliveryNoteDetailService from "./service";
import { Request, Response, NextFunction } from "express";
import { CreateDto } from "./dtos/create.dto";
import message from "@core/config/constants";

export class DeliveryNoteDetailController {
    public service = new DeliveryNoteDetailService();
    ...
    public searchs = async (req: Request, res: Response, next: NextFunction) => {
        ...
        const qty = req.query.qty as any;
        ...
        try {
            const result = await this.service.searchs(key, name, qty, pageInt, limitInt);
            if (result instanceof Error && result.field)
            return sendResponse(res, result.status, result.message, null, result.field);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
