...
import PurchaseOrderStatusService from "./service";
import { Request, Response, NextFunction } from "express";
import { CreateDto } from "./dtos/create.dto";
import message from "@core/config/constants";

export class PurchaseOrderStatusController {
    public service = new PurchaseOrderStatusService();
    ...
    public updateOrderStatus = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const result = await this.service.updateOrderStatus(order_id, user_id as number, status as string);
            if (result instanceof Error) {
                return sendResponse(res, result.status, result.message);
            }
            return sendResponse(res, 200, message.UPDATE_SUCCESS, result);
