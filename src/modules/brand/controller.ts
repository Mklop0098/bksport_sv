...
export class BrandController {
    public service = new BrandService();
    ...
    public gennderBrand = async (req: Request, res: Response, next: NextFunction) => {
        let created_id: number = req.id as any;
        try {
            const result = await this.service.gennderBrand(created_id);
            if (result instanceof Error)
                return sendResponse(res, result.data as any, result.message);
            return sendResponse(res, 200, message.UPDATE_SUCCESS, result);
        } catch (error) {
            next(error);
        }
