...
class ProductIamgeService {
    private tableName = 'product_image';
    private fieldId = 'id'

    public create = async (model: CreateDto) => {
        let query = `insert into ${this.tableName} (product_id, image, publish, created_id) values (?, ?, ?, ?)`;
        let values = [
            model.product_id,
            model.image,
            model.publish,
    ...
    public delete = async (id: number) => {
       //console.log(id)
        ...
        const query = `SELECT pi.image, p.code, p.parent_id FROM product_image pi LEFT JOIN product p ON pi.product_id = p.id WHERE pi.id = ?`
        ...
        await this.deleteImage(path.resolve(__dirname, process.env.PRODUCT_UPLOAD_IMAGE_PATH + `/${code}/thumbnail`), image)
        await this.deleteImage(path.resolve(__dirname, process.env.PRODUCT_UPLOAD_IMAGE_PATH + `/${code}`), image);
    ...
    public deleteByProductId = async (id: number) => {
        const exist = await checkExist(this.tableName, 'product_id', id);
        try {
            if (exist.length > 0) {
                let query = `delete from ${this.tableName} where product_id = ?`
    ...
    public findAllProductImageByProductId = async (id: number) => {
        const result = await database.executeQuery(`select * from ${this.tableName} where product_id = ?`, [id]);
        if (Array.isArray(result) && result.length === 0)
            return new HttpException(404, errorMessages.NOT_FOUND);
        if (Array.isArray(result) && result.length > 0) {
    ...
    // private uploadImage = async (image: string, code: string) => {
    //     let userDir = path.join(process.env.PRODUCT_UPLOAD_IMAGE as string, code);
    //     let userDirThumbnail = path.join(process.env.PRODUCT_UPLOAD_IMAGE as string, code, 'thumbnail');
    //     let pathImage = path.join(userDir, image);
    //     let pathImageThumbnail = path.join(userDirThumbnail, image);
    //     return {
    //         pathImage: pathImage,
    //         pathImageThumbnail: pathImageThumbnail
    //     }
    // }
    // private uploadImageProductId = async (files: any, id: number, model: CreateDto) => {
    //     const check = await checkExist('product', 'id', id.toString());
    //     let quantity: number
    //     if (check == false) return new HttpException(404, errorMessages.NOT_FOUND);
    //     const checkQuantity = await database.executeQuery(`select count(*) as total from product_image where product_id = ?`, [id]);
    //     if (Array.isArray(checkQuantity) && checkQuantity.length > 0 && (checkQuantity as RowDataPacket[])[0].total + files.length > process.env.PRODUCT_IMAGE_QUANTITY! as any as number)
    //         return new HttpException(400, errorMessages.INVALID_FILE_QUANTITY);
    //     if(Array.isArray(checkQuantity) && checkQuantity.length === 0)
    //         quantity = 0
    //     else
    //         quantity = (checkQuantity as RowDataPacket[])[0].total
    //     let query = `insert into product_image (product_id, image, publish, created_id, created_at, updated_at) values `
    //     let values = []
    //     const code = (check as RowDataPacket[])[0].code
    //     const userDir = path.join(process.env.PRODUCT_UPLOAD_IMAGE as string, code);
    //     const userDirThumbnail = path.join(process.env.PRODUCT_UPLOAD_IMAGE as string, code, 'thumbnail');
    //     if (!files) return new HttpException(400, errorMessages.INVALID_FILE);
    //     if (!Array.isArray(files)) return new HttpException(400, errorMessages.INVALID_FILE);
    //     if (files.length === 0) return new HttpException(400, errorMessages.INVALID_FILE);
    //     if(Array.isArray(files) && files.length > 0) {
    //         for (let i = 0; i < files.length; i++) {
    //             query += `(?, ?, ?, ?, ?, ?), `
    //             values.push(id)
    //             values.push(files[i].filename)
    //             values.push(1)
    //             values.push(id)
    //             values.push(new Date())
    //             values.push(new Date())
    ...
    private uploadImage = async (file: Express.Multer.File, dir: string, productName: string, num: number) => {
        ...
        try {
            ...
            const imageName = await this.convertNameImage(productName, num) + fileExtension;
    ...
    public uploadOneImage = async (file: any, model: CreateDto) => {
        ...
        if (Array.isArray(checkQuantity) && checkQuantity.length > 0 && (checkQuantity as RowDataPacket[])[0].total + 1 > process.env.PRODUCT_IMAGE_QUANTITY! as any as number)
            return new HttpException(400, errorMessages.INVALID_FILE_QUANTITY);
        const lastRow = await database.executeQuery(`select * from product_image where product_id = ? order by id desc limit 1`, [id]);
...
export default ProductIamgeService;