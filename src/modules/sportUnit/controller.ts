...
import SportUnitService from "./service";
import { Request, Response, NextFunction } from "express";
import { CreateDto } from "./dtos/create.dto";
import message from "@core/config/constants";

export class SportUnitController {
    public service = new SportUnitService();
    ...
    public searchs = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const result = await this.service.searchs(key, name, publish, page, limit, seller_id);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
    ...
    public updateLocalUnit = async (req: Request, res: Response, next: NextFunction) => {
        const id: number = req.params.id as any;
        try {
            const result = await this.service.updateLocalUnit(id);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
            return sendResponse(res, 200, message.UPDATE_SUCCESS, result);
        } catch (error) {
