...
class SportUnitService {
    private tableName = 'sport_unit';
    ...
    public searchs = async (key?: string, name?: string, publish?: boolean, page?: number, limit?: number, seller_id?: number) => {
        ...
        if (publish) {
            query += ` and (publish = ${publish} or local_unit = 1)`
            countQuery += ` and (publish = ${publish} or local_unit = 1)`
        }
        if (seller_id != undefined && seller_id != 1    ) {
            query += ` and (seller_id = ${seller_id} or local_unit = 1)`
            countQuery += ` and (seller_id = ${seller_id} or local_unit = 1)`
        }
    ...
    public updateLocalUnit = async (id: number) => {
        const exist = await checkExist(this.tableName, 'id', id.toString());
        if (exist == false)
            return new HttpException(404, errorMessages.NOT_EXISTED, 'id');
        let result
        if (exist[0].local_unit == 1) {
            result = await database.executeQuery(`update ${this.tableName} set local_unit = 0 where id = ?`, [id]);
        }
        else {
            result = await database.executeQuery(`update ${this.tableName} set local_unit = 1 where id = ?`, [id]);
        }
        if ((result as any).affectedRows === 0)
            return new HttpException(400, errorMessages.UPDATE_FAILED);
        return {
...
export default SportUnitService;