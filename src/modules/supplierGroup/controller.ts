...
import SupplierGroupService from "./service";
import { Request, Response, NextFunction } from "express";
import { CreateDto } from "./dtos/create.dto";
import message from "@core/config/constants";

export class CustomerGroupController {
    public service = new SupplierGroupService();
    ...
    public searchs = async (req: Request, res: Response, next: NextFunction) => {
        ...
        try {
            const result = await this.service.searchs(key, name, publish, pageInt, limitInt, code, seller_id);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
    ...
    public updateListPublish = async (req: Request, res: Response, next: NextFunction) => {
        ...
        model.created_id = req.id;
        try {
            const result = await this.service.updateListPublish(model, listId, publish);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
            return sendResponse(res, 200, message.UPDATE_SUCCESS, result);
        } catch (error) {
