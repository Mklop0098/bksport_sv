...
export class UserRoleController {
    public service = new UserRoleService();
    ...
    public searchs = async (req: Request, res: Response, next: NextFunction) => {
        const user_id: number = req.query.user_id as any;
        ...
        const seller_id = 0
        try {
            const result = await this.service.searchs(key, user_id, role_id, pageInt, limitInt, seller_id);
            if (result instanceof Error && result.field)
                return sendResponse(res, result.status, result.message, null, result.field);
            if (result instanceof Error)
                return sendResponse(res, result.status, result.message);
    ...
    public getRoleByUserId = async (req: Request, res: Response, next: NextFunction) => {
        const user_id: number = req.id as any;
        try {
            const result = await this.service.getRoleByUserId(user_id);
            if (result instanceof Error)
                return sendResponse(res, 404, result.message);
            return sendResponse(res, 200, message.FIND_ALL_SUCCESS, result);
        } catch (error) {
