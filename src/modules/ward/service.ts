...
class CityService {
    ...
    public create = async (model: CreateDto) => {
        ...
        const result = await database.executeQuery(`insert into ${this.tableName} (name, publish, created_at, updated_at, district_id) values (?, ?, ?, ?, ?)`, [model.name, publish, created_at, updated_at, model.district_id]);
    ...
    public update = async (model: CreateDto, id: number) => {
        ...
        const result = await database.executeQuery(`update ${this.tableName} set name = ?, publish = ?, updated_at = ?, district_id = ? where id = ?`, [model.name, model.publish, update_at, model.district_id, id]);
        if ((result as any).affectedRows === 0)
    ...
    public findById = async (id: number) => {
        ...
        //     if (this.isNumeric(element.name)) {
        //         element.name = 'Phường ' + element.name;
        //     }
        // })
        return {
    ...
    public searchs = async (key: string, name: string, publish: boolean, page: number, limit: number, district_id: number) => {
        ...
        if (district_id) {
            ...
            countQuery += ` and district_id = ${district_id}`
        }
