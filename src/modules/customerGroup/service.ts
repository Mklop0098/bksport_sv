...
class CustomerGroupService {
    ...
    public create = async (model: CreateDto) => {
        ...
        if (code && code.length > 0) {
            ...
            code = await generateCodePrefixChar(this.tableName, 'CNT', 8)
        }
        const created_at = new Date()
        const updated_at = new Date()
        const result = await database.executeQuery(`insert into ${this.tableName} (name, publish, created_id, created_at, updated_at, code, seller_id) values (?, ?, ?, ?, ?, ?, ?)`, [model.name, model.publish ?? 1, model.created_id, created_at, updated_at, code, model.seller_id]);
    ...
    public searchs = async (key: string, name: string, publish: boolean, page: number, limit: number, code: string, is_default: boolean, seller_id: number) => {
        let query = `select * from ${this.tableName} where 1=1`;
        let countQuery = `SELECT COUNT(*) as total FROM ${this.tableName} WHERE 1=1`;
        if (key != undefined) {
            query += ` and (code like '%${key}%' or name like '%${key}%')`
            countQuery += ` and (code like '%${key}%' or name like '%${key}%')`
        }
        if (name != undefined) {
